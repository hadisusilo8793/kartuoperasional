document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await window.app.api.fetch('/api/armada');
    ...
    const renderTable = (data) => {
        tableBody.innerHTML = '';
        if (data.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="5" class="text-center py-4">Tidak ada data armada.</td></tr>`;
            return;
        }
        data.forEach(item => {
    ...
    const handleSave = async (row) => {
        ...
        const isNew = !id;
        const data = {
            nomor_armada: row.querySelector('[name="nomor_armada"]').value,
            jenis: row.querySelector('[name="jenis"]').value,
            plat: row.querySelector('[name="plat"]').value,
            status: row.querySelector('[name="status"]').value,
        };
        try {
            const url = isNew ? '/api/armada' : `/api/armada/${id}`;
