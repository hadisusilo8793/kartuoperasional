document.addEventListener('DOMContentLoaded', () => {
    ...
    const createFormHtml = (item = {}) => `
        <td class="px-6 py-4"><input type="text" name="kode" value="${item.kode || ''}" class="form-input-sm uppercase-input" required></td>
        <td class="px-6 py-4"><input type="text" name="nama" value="${item.nama || ''}" class="form-input-sm" required></td>
        <td class="px-6 py-4">
            <select name="kategori" class="form-input-sm uppercase-input">
    ...
    tableBody.addEventListener('click', async (e) => {
        ...
        if (editBtn) {
            ...
            const id = row.dataset.id;
            try {
                const res = await window.app.api.fetch(`/api/gate?id=${id}`);
                const item = res.data[0];
                row.dataset.original = JSON.stringify(item);
                row.innerHTML = createFormHtml(item);
                lucide.createIcons();
            } catch (error) {
                window.app.ui.showToast('Gagal mengambil data gate.', 'error');
            }
        }
        if (deleteBtn) {
            ...
            if (confirm('Apakah Anda yakin ingin menghapus gate ini?')) {
                try {
                    const res = await window.app.api.fetch(`/api/gate/${row.dataset.id}`, { method: 'DELETE' });
